...
clangの中にassemblyを埋め込む記法。  
clangの変数とassemblyを連携できる。  
...
```
__asm__ __volatile__("<assembly>" : 出力operand : 入力operand : 破壊するregister)
;
```

* 出力operand
  * assemblyの処理結果をどこに取り出すかの宣言
  * "制約" (Cの変数名)
    * `=`はassemblyで変更されること
...
* operandは`0%`,`1%`と順番にassemblyから参照できる
